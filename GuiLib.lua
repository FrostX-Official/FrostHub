...
function UILibrary.Load(GUITitle)
	...
	function TabLibrary.AddPage(PageTitle, SearchBarIncluded)
		...
		function PageLibrary.AddToggle(Text, Default, Callback)
			...
			function ToggleLib.Switch()
				if ThisToggle == true then
					ThisToggle = false
					RightTick.ImageTransparency = 1
					EffectFrame.BackgroundColor3 = Color3.fromRGB(255,160,160)
					Callback(ThisToggle)
				else
					ThisToggle = true
					EffectFrame.BackgroundColor3 = Color3.fromRGB(0,255,109)
					RightTick.ImageTransparency = 0
					Callback(ThisToggle)
				end
			end
